def to_string(value, indent=2):
    ...
    if isinstance(value, dict):
        ...
        for key, val in value.items():
            lines.append(f'{" " * indent}  {key}: {to_string(val, indent + 4)}\n')
        return f'{{\n{"".join(lines)}{" " * (indent-2)}}}'
    return value
...
def stylish(dic, indent=2):
    ...
    for key, value in dic.items():
        if value['type'] == 'removed':
            result.append(f'{" " * indent}- {key}: {to_string(value["value"], indent+4)}')
        elif value['type'] == 'added':
            result.append(f'{" " * indent}+ {key}: {to_string(value["value"], indent+4)}')
        elif value['type'] == 'updated':
            result.append(f'{" " * indent}- {key}: {to_string(value["value1"], indent+4)}')
            result.append(f'{" " * indent}+ {key}: {to_string(value["value2"], indent+4)}')
